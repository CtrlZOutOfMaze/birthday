...
import CardStackWidget from "@/components/card-stack"
import FloatingHearts from "@/components/floating-hearts"
import Loader from "@/components/Loader"
import ScratchGift from "@/components/gift-card"
...
export default function Home() {
  ...
  const startSratchGift = () => {
    setShowForYouBtn(false)
    setShowGiftForYouBtn(false)
    setIsBirthday(true)
    // Play the song
    if (audioRef.current) {
      audioRef.current.volume = 0.8;
    ...
    <main className="min-h-screen bg-gradient-to-b from-rose-100 to-purple-100 flex flex-col items-center justify-center p-4 overflow-hidden">
      ...
      >
        <motion.div className="bg-white bg-opacity-80 backdrop-blur-sm rounded-3xl shadow-xl shadow-rose-100 p-8 border-2 border-rose-200"
          ...
          <AnimatePresence mode="wait">
            {isBirthday ? (
              showForYouBtn ? <CardStackWidget key="celebration" /> : showGiftForYouBtn ? <BirthdayCelebration key="celebration" />:<ScratchGift  key="celebration"/>
            ) : (
              <Countdown key="countdown" targetDate={birthdayDate} onCountdownEnd={() => setShowAboutYouBtn(true)} />
            )}
          </AnimatePresence>
        </motion.div>
      </motion.div>

      {showAboutYouBtn && <motion.div
        key="start-button"
      ...
      >
        ...
        >
          ...
          <span className="text-xl">Tap to celebrate!</span>
      ...
      >
        ...
        >
          <PartyPopper className="w-6 h-6" />
          <span className="text-xl">Note for you</span>
          <MoveRight className="w-5 stroke-3 h-6" />
        </motion.button>
      </motion.div>}

      {showGiftForYouBtn && <motion.div
        key="start-button"
        className="flex flex-col items-center justify-center mt-8"
        initial={{ opacity: 0, y: -30 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        ...
        >
          <PartyPopper className="w-6 h-6" />
          <span className="text-xl">A Small Gift!</span>
          <MoveRight className="w-5 stroke-3 h-6" />
        </motion.button>
      </motion.div>}
